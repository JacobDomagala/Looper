...
Enemy::Shoot()
{
   ...
   if (glm::length(static_cast< glm::vec2 >(Game::GetInstance().GetPlayer().GetCenteredLocalPosition() - m_centeredLocalPosition))
       <= m_weapon->GetRange())
   {
      if (m_timeSinceLastShot >= m_weapon->GetReloadTime())
      {
         auto collided = Game::GetInstance().CheckBulletCollision(this, Game::GetInstance().GetLevel().GetGlobalVec(m_targetShootPosition),
                                                                  m_weapon->GetRange());
...
Enemy::ReturnToInitialPosition()
{
   ...
   {
      m_currentNodeIdx = Game::GetInstance().GetLevel().GetPathfinder().FindNodeIdx(m_centeredLocalPosition);
      m_targetMovePosition =
         Game::GetInstance().GetLevel().GetPathfinder().GetNearestPosition(/*m_centeredLocalPosition*/ m_currentNodeIdx, m_initialPosition);
      auto move = m_targetMovePosition - m_centeredLocalPosition;
      auto nMove = glm::normalize(static_cast< glm::vec2 >(move));
...
void
Enemy::Render(const Shaders& program)
{
